...
public class build : MonoBehaviour {
	...
	void OnGUI () {
		...
		//Debug.Log (X);
		//Debug.Log (Posx);
		if ( ev.Equals(Event.KeyboardEvent("a")) && Posx-2 <= X && X <= Posx+ 2 && Y <= Posy + 1 && Posy -1 <= Y && Allreadybuilded == false && Player.Ressource.gold >= 200 ){
			//Debug.Log ("LOOOOOOOL");
			Player.Ressource.gold -= 200;
			var MonObjet = creer("Bat1","Texture/Bat/FUCK IT");
			deplacer (MonObjet, Posx, Posy + 1.6f, -1f);
			Allreadybuilded = true;
			Buildings.gold += 1;
		}
		else if (ev.Equals(Event.KeyboardEvent("z")) && Posx-2 <= X && X <= Posx+ 2 && Y <= Posy + 1 && Posy -1 <= Y && Allreadybuilded == false && Player.Ressource.gold >= 200){
			//Debug.Log ("LOOOOOOOL");
			Player.Ressource.gold -= 200;
